...
def init_db() -> None:
    ...
    try:
        ...
        try:
            ...
            pass
        # 添加 admin_rate_set_by 列（记录是哪个管理员设置的费率，如果是超级管理员设置则为'super_admin'）
        try:
            cur.execute("ALTER TABLE user_data ADD COLUMN IF NOT EXISTS admin_rate_set_by VARCHAR")
        except:
            pass
        # 🔥 用户固定 token：保存明文 token（用于“token不变”的需求）
        try:
        ...
        try:
            ...
            if not cur.fetchone():
                ...
                logger.info("Initialized default server manager password to '1'.")
        except Exception:
            pass
...
def _issue_user_token(conn, user_id: str) -> str:
    """
    需求：token 不变。
    - 第一次生成后写入 user_data.auth_token_plain 并复用
    - user_tokens 里只存 hash（expires_at 置 NULL，不做过期）
    """
    cur = conn.cursor()
    ...
    try:
        cur.execute("SELECT auth_token_plain FROM user_data WHERE user_id=%s", (user_id,))
        row = cur.fetchone()
        if row:
            token = row[0] if not isinstance(row, dict) else row.get("auth_token_plain")
    except Exception:
        token = None
    ...
    if not token:
        ...
        try:
            cur.execute("UPDATE user_data SET auth_token_plain=%s WHERE user_id=%s", (token, user_id))
        except Exception:
            # 如果列不存在（极少情况），忽略，仍返回 token（但后续将会重新生成）
            pass
...
def admin_users_collection():
    ...
    try:
        ...
        cur2 = conn.cursor()
        cur2.execute("INSERT INTO users(user_id, username, pw_hash, created_by_admin) VALUES(%s,%s,%s,%s)", (uid, username, hash_pw(password), admin_id))
